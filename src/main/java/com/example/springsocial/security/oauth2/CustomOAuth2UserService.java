...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    private OAuth2User processOAuth2User(OAuth2UserRequest oAuth2UserRequest, OAuth2User oAuth2User) {
        ...
        if(userOptional.isPresent()) {
            ...
            if(!applicationUser.getProvider().equals(Provider.valueOf(oAuth2UserRequest.getClientRegistration().getRegistrationId()))) {
                throw new OAuth2AuthenticationProcessingException("Looks like you're signed up with " +
                        applicationUser.getProvider() + " account. Please use your " + applicationUser.getProvider() +
                        " account to login.");
            }
            updateExistingUser(applicationUser, oAuth2UserInfo);
        } else {
    ...
    private ApplicationUser registerNewUser(OAuth2UserRequest oAuth2UserRequest, OAuth2UserInfo oAuth2UserInfo) {
        ...
        applicationUser.setProvider(Provider.valueOf(oAuth2UserRequest.getClientRegistration().getRegistrationId()));
    ...
    private void updateExistingUser(ApplicationUser existingUser, OAuth2UserInfo oAuth2UserInfo) {
        existingUser.setName(oAuth2UserInfo.getName());
        existingUser.setImageUrl(oAuth2UserInfo.getImageUrl());
        applicationUserRepository.updateName(existingUser.getId(), existingUser.getName());
    }
